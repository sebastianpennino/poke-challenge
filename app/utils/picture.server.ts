 ...
 */
export const getPokemonPicFromExternalAPI = async (
  name: string,
): Promise<string> => {
  ...
  try {
    ...
    console.log(
      `Error fetching image for ${normalizedName} returning the default image`,
    );
    // default image
...
export const fetchAllPokemonPics = async (
  pokemonNames: string[],
): Promise<string[]> => {
  try {
    const fetchPromises = pokemonNames.map((name) =>
      getPokemonPicFromExternalAPI(name),
    );
